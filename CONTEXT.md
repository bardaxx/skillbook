...
### OpenSpec program (cross-repo workflow)

**Program register**  
A markdown planning file (usually `openspec/programs/<slug>.md` in a consumer repo) listing slices, priorities, and lifecycle status. It is not an OpenSpec change.  
_Avoid_: timeline (as product name), backlog file

**Slice**  
One decomposed unit of work inside a program register, with an id (`T01`, `L02`, `F03`) and a candidate OpenSpec change id.  
_Avoid_: ticket, story (unless the consumer’s issue tracker uses those terms externally)

**Slice prefix**  
`T` (behavior/coverage), `L` (legacy/stabilization), `F` (product feature). Part of the slice id, not a separate entity.  
...
Files in a consumer repo that tell every agent session how to work (commonly `AGENTS.md`, sometimes mirrored elsewhere). The program layer must be documented there when `openspec-program` is adopted.  
...
- A **consumer repo** may install one or more skills and optionally maintain a **program register**.
...
| Term | Resolution |
|------|------------|
| “timeline” | Legacy filename (`TIMELINE_TEST.md`) or informal progress log only. Product language is **program** / **program register**. |
